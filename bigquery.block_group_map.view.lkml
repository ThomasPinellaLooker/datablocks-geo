view: bq_logrecno_bg_map {
  ...
  derived_table: {
    sql:
      ...
      FROM
        `lookerdata.acs_fast_facts.geo_2015` as geo
      LEFT JOIN `lookerdata.acs_fast_facts.block_group_attribs` as bg on (SUBSTR(geo.GEOID, 8, 12) = bg.geoid AND geo.SUMLEVEL = '150')
      LEFT JOIN `lookerdata.acs_fast_facts.block_group_attribs` as tr on (SUBSTR(geo.GEOID, 8, 11) = SUBSTR(tr.geoid, 1, 11) AND geo.SUMLEVEL = '140')
      WHERE
        sumlevel in ('140', '150')
